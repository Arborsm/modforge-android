...
public sealed partial class NexusService
{
    ...
    public Task<JsonElement?> DownloadLauncherModAsync(JsonElement request)
    {
        return Task.Run<JsonElement?>(async () =>
        {
            ...
            try
            {
                var totalBytes = await DownloadToArchiveAsync(linkUri!, archivePath, downloadId, CancellationToken.None).ConfigureAwait(false);
    ...
    async Task<long> DownloadToArchiveAsync(string url, string archivePath, string downloadId, CancellationToken cancellation)
    {
        using var response = await NexusClient.GetAsync(url, cancellation).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode)
            throw new LauncherCommandException("network", $"Failed to download launcher mod: HTTP {(int)response.StatusCode}");

        var totalBytes = response.Content?.Headers.ContentLength;
        var buffer = new byte[81920];
        long downloadedBytes = 0;
        var lastEmit = Environment.TickCount64;
        await using var source = response.Content == null
            ? throw new LauncherCommandException("network", "Download response has no content.")
            : await response.Content.ReadAsStreamAsync(cancellation).ConfigureAwait(false);
        await using var destination = new FileStream(archivePath, FileMode.Create, FileAccess.Write);
        int read;
